...
class RealsenseCompressedNode(LifecycleNode):
    ...
    def on_activate(self, state):
        ...
        if self.robot_model == 'k1':
            ...
            # K1 has no back camera — use depth visual if available, otherwise raw depth
            depth_topic = '/depth/image'
            self.get_logger().info(f"K1 depth topic: {depth_topic}")
            self.subscription_back = self.create_subscription(Image, depth_topic, self.depth_callback, k1_qos)
            self.get_logger().info(f'Node activated and subscribed to /rgb/image and {depth_topic} (K1 stereo camera).')
        else:
    ...
    def image_callback_back(self, msg):
        ...
        try:
            ...
            # Resize and compress the image
            small_frame = cv2.resize(frame, (640, 360))
            ret_enc, jpeg = cv2.imencode('.jpg', small_frame, [cv2.IMWRITE_JPEG_QUALITY, 40])
        ...
        try:
            if msg.encoding in ('bgr8', 'rgb8', 'bgra8', 'rgba8'):
                ...
                if d_max > d_min:
                    ...
                    self.get_logger().warn('depth_callback: flat depth frame (d_min==d_max), publishing black image')
                    normalized = np.zeros_like(frame)
                frame = cv2.applyColorMap(((1.0 - normalized) * 255).astype(np.uint8), cv2.COLORMAP_INFERNO)
            ...
            self.get_logger().error(f'depth_callback exception: {e}', exc_info=True)
